...
def _write_task_payload_file(task_data: dict, execution_mode: str) -> Path:
    """
    Gera o JSON temporário que será enviado ao bot.

    Esse é o arquivo que o bot recebe como argumento:
    python main.py C:\\OrkaFlow\\tmp\\task_<id>.json
    """
    ...
    if not task_id:
        raise RuntimeError("task_id não encontrado para gerar payload temporário.")
